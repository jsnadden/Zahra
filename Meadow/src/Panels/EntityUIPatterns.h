...
namespace Zahra
{
	...
	public:
		...
		// TODO: refactor this whole thing so that DrawComponent sets up a SINGLE table, and populates
		// it with [label | control] pairs, rather than making a new imgui table for each row lol

		static void DrawFloatControl(const std::string& label, float& value, float speed = .05f,
		...
		static void DrawVec2Controls(const std::string& label, glm::vec2& values, float resetValue = .0f, float speed = .05f,
			bool logarithmic = false, float min = -FLT_MAX / INT_MAX, float max = FLT_MAX / INT_MAX, float columnWidth = 90.f)
		{
			ImGuiSliderFlags flags = 0;
